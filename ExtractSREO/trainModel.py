...
label_pipeline = None
model = None
COLUMN_LABELS = {0: "N/A", 1: "Units", 2: "City", 3: "State", 4: "Street Address", 5: "Rate Type", 6: "Year Acquired", 7: "Maturity"}
...
def get_column_label(label):
    if label == "Maturity": return 7
    elif label == "Year Acquired": return 6
    elif label == "Rate Type": return 5
    elif label == "Street Address": return 4
...
def trainModel(columnOrHeader, trainingFilePath, testingFilePath):
    ...
    if(newModel == "y"):
        ...
        modelName = input("Existing Model Name: ")
        loadModel(modelName)
    ...
    torch.save(model, "Model/" + modelName + ".pt")
    torch.save(vocab, "Model/" + modelName + "Vocab.pt")
    print("Saved to path: Model/" + modelName + ".pt")
...
def loadModel(modelName):
    ...
    global text_pipeline
    global label_pipeline
    vocab = torch.load("Model/" + modelName + "Vocab.pt")
    model = torch.load("Model/" + modelName + ".pt")
    model.eval()
    text_pipeline = lambda x: vocab(tokenizer(x))
    label_pipeline = lambda x: int(x)
...
def testInput(modelName, columnOrHeader, testString, printConfirm):
    ...
    if(printConfirm == 1): 
        print("[N/A, Units, City, State, Address, Rate Type, Acquisition Date]")
        probsString = ""
        for i in range(len(probs[0])):
